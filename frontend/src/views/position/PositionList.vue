...
const loadData = async () => {
  ...
  try {
    ...
    // 初始化人员数量统计
    positionUserCounts.value = mockData.reduce((acc, item) => {
      acc[item.id] = item.userCount
      return acc
    }, {})
  } catch (error) {
...
const loadDepartments = async () => {
  try {
    ...
    departmentList.value = flattenDepartments(mockDepartments)
    departmentTreeData.value = formatDepartmentTree(mockDepartments)
  } catch (error) {
