  ...
  <div class="account-management-container">
    ...
    <div class="account-list-section">
      ...
      >
        ...
        <el-table-column prop="assignedPerson" label="分配人员" width="160">
          <template #default="{ row }">
...
const totalAccounts = computed(() => totalUsers.value)
...
const toggleAccountStatus = async (account) => {
  try {
    ...
    await updateUserStatus(account.id, newStatus)
...
const confirmResetPassword = async () => {
  try {
    await resetUserPassword(currentAccount.value.id, passwordForm.value.password)
...
const loadUsers = async () => {
  try {
    ...
    if (response.code === 200) {
      accounts.value = response.data.content || response.data.records || response.data.list || []
      totalUsers.value = response.data.totalElements || response.data.total || 0
    } else {
